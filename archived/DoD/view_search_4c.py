...
def normalize(df):
    for c in df.columns:
        df[c] = df[c].apply(lambda x: str(x).lower())
    return df
...
def identify_compatible_groups2(dataframes_with_metadata):
    compatible_groups = []
    hash_view = {} #{hash_value: [view_path1, view_path2]}

    for t, path, md in dataframes_with_metadata:
        hashes = hash_pandas_object(t, index=False)
        ht = hashes.sum()
        if ht in hash_view.keys():
            hash_view[ht].append(path)
        else:
            hash_view[ht] = [path]

    for _, views in hash_view.items():
        if len(views) > 1:
            compatible_groups.append(views)
...
def chasing_4c(dataframes_with_metadata):
    ...
    compatible_groups = identify_compatible_groups2(dataframes_with_metadata)
    ...
    return compatible_groups, contained_groups
...
def group_into_one_schema(dataframes):
    schema_id_info = dict()
    schema_id = 0
    schema_to_dataframes = defaultdict(list)
    for df, path in dataframes:
        schema_id_info[schema_id] = 0
        schema_to_dataframes[schema_id].append((df, path))
    return schema_to_dataframes, schema_id_info
...
def perform4c(dfs):
    ...
    for key, group_dfs in dfs_per_schema.items():
        ...
        compatible_group, contained_group = chasing_4c(dfs_with_metadata)
