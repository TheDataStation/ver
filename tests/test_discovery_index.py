...
def test_load_data(clean_up):
    ...
    discovery_graph = DiscoveryIndex(ProfileIndexDuckDB(config), GraphIndexKuzu(config))
...
def test_chain_graph_duckdb():
    ...
    discovery_graph = DiscoveryIndex(ProfileIndexDuckDB(config), GraphIndexDuckDB(config))

    for i in range(5):
        discovery_graph.add_node({"id": i, "attr": 1})
    for i in range(4):
        discovery_graph.add_undirected_edge(i, i + 1, EdgeType.ATTRIBUTE_SYNTACTIC_SIMILARITY, {"weight": 1})
