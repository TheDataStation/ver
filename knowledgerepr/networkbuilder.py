...
def build_content_sim_mh_text(network, mh_signatures, t):
    f = open('create_fk_postgres.sql', 'w')
    def connect(nid1, nid2, score):
    ...
    for nid, mh_obj in mh_sig_obj:
        ...
        for r_nid in res:
            if r_nid != nid:
                ...
                table2 = sn2[:-4] + "_csv"
                fk_num+=1
                constraint_name = "fk_" + str(fk_num)
                postgres_sql = "ALTER TABLE " + table1 + " ADD CONSTRAINT " + constraint_name + " FOREIGN KEY(\"" + fn1 \
                               + "\") REFERENCES " + table2 + "(\"" + fn2 + "\");\n"
